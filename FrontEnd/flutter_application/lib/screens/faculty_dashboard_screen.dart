...
class _FacultyDashboardScreenState extends State<FacultyDashboardScreen>
  ...
  List<Map<String, Object?>> _menuItems() {
    return [
      {'icon': Icons.dashboard_rounded, 'label': 'Dashboard', 'active': true, 'route': '/faculty/dashboard'},
        ...
        children: [
          ...
          if (_isLoadingSchedule)
            ...
            ..._todaySchedule.map((entry) {
              final slot = _allTimeSlots.where((s) => s.id == entry.timeSlotId).firstOrNull ??
                  TimeSlot(
                      id: '',
                      startTime: '--',
                      endTime: '--',
              ...
              final course = _allCourses.where((c) => c.courseCode == entry.courseCode).firstOrNull ??
                  Course(
              ...
              final room = _allRooms.where((r) => r.id == entry.roomId).firstOrNull ??
                  Room(
                      id: entry.roomId,
                      name: entry.roomId,
                      capacity: 0,
